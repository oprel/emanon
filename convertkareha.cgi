...
foreach (@threadlist) {
    my $thread = $_;
    open my $read, '<', "res/$thread" || die "Cannot read static file: $!";
    flock $read, LOCK_SH;
    my @raw = <$read>;
    my $html = join('',@raw);
    close $read;
    
my $thread_id = substr($thread, 0, -5);
...
$html =~ s/<\/div> <\/div>  <form.*/($footer)/;
    ...
    open my $write, '>', "converted/$thread" || die "Cannot write file: $!";
    flock $write, LOCK_EX;
    print $write "$html";
    close $write;
    print "Finished: $thread<br>";
} 
print "Content-type: text/html\n\n" . 'DONE!';

sub read_dir($) {
    my $folder = shift;
    opendir my $read, "$folder/" || die "Cannot open find res folder: $!";
